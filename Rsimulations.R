...
totals[1]=NA #don't forget to remove!

source("freeimputations.R")
source("initialvalues.R")
source("sampleimp.R")
source("lintoolsadjustment seq.R")
...
holecycle=holebatches*ceiling(rubindatasets/parallelimp)
it=0
...
repeat {
  ...
  if (((it/parallelimp)-1)%%(holecycle*datbatches)==0) {
    set.seed(seeddat)
  ...
  if (((it/parallelimp)-1)%%holecycle==0) {
    ...
    nassim11=sample(prod(dim(holessim11)),frac1*prod(dim(truevalssim1)))
    nassim12=sample(prod(dim(holessim12)),frac2*prod(dim(truevalssim1)))
    nassim13=sample(prod(dim(holessim13)),frac3*prod(dim(truevalssim1)))
    nassim21=sample(prod(dim(holessim21)),frac1*prod(dim(truevalssim2)))
    nassim22=sample(prod(dim(holessim22)),frac2*prod(dim(truevalssim2)))
    nassim23=sample(prod(dim(holessim23)),frac3*prod(dim(truevalssim2)))
    ...
    datafreesim11=freeimp(datasim11,rules,totals,wts)
    datafreesim12=freeimp(datasim12,rules,totals,wts)
    datafreesim13=freeimp(datasim13,rules,totals,wts)
    datafreesim21=freeimp(datasim21,rules,totals,wts)
    datafreesim22=freeimp(datasim22,rules,totals,wts)
    datafreesim23=freeimp(datasim23,rules,totals,wts)
    holes1=holes2=holes3=as.matrix(truevals[,vars])
    nas1=sample(prod(dim(holes1)),frac1*prod(dim(truevals)))
    nas2=sample(prod(dim(holes2)),frac2*prod(dim(truevals)))
    nas3=sample(prod(dim(holes3)),frac3*prod(dim(truevals)))
    holes1[nas1]=NA
    holes2[nas2]=NA
    holes3[nas3]=NA
    ...
    imputedsim111=imputedsim112=imutedsim113=imputedsim121=imputedsim122=imputedsim123=imputedsim131=imputedsim132=imputedsim133=imputedsim211=imputedsim212=imputedsim213=imputedsim221=imputedsim222=imputedsim223=imputedsim231=imputedsim232=imputedsim233=list()
    imputedsimcont11=imputedsimcont12=imputedsimcont13=imputedsimcont21=imputedsimcont22=imputedsimcont23=list()
    imputed11=imputed12=imputed13=imputed21=imputed22=imputed23=imputed31=imputed32=imputed33=list()
    imputedcont1=imputedcont2=imputedcont3=list()
    if (givefeedback) {
  ...
  if (length(imputedsim111)>=rubindatasets) {
    ...
    for (xa in 1:(length(imputedsim111)%/%rubindatasets)) {
      ...
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed11[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits1,datinpool=rubindatasets,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed12[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits2,datinpool=rubindatasets,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed13[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits3,datinpool=rubindatasets,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed21[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits1,datinpool=rubindatasets,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed22[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits2,datinpool=rubindatasets,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed23[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits3,datinpool=rubindatasets,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed31[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits1,datinpool=rubindatasets,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed32[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits2,datinpool=rubindatasets,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed33[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits3,datinpool=rubindatasets,logtrans = TRUE)
      
      #rubin-validating real control
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont1[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree1,truevals,response,explanatory,frac=frac1,maxits=1,datinpool=rubindatasets,logtrans = TRUE)
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont2[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree2,truevals,response,explanatory,frac=frac2,maxits=1,datinpool=rubindatasets,logtrans = TRUE)
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont3[(1+(xa-1)*rubindatasets):(xa*rubindatasets)],datafree3,truevals,response,explanatory,frac=frac3,maxits=1,datinpool=rubindatasets,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data1,truevals,response,explanatory,frac=frac1,maxits=0,datinpool=rubindatasets,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data2,truevals,response,explanatory,frac=frac2,maxits=0,datinpool=rubindatasets,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data3,truevals,response,explanatory,frac=frac3,maxits=0,datinpool=rubindatasets,logtrans = TRUE)
    }
    ...
    for (xb in 1:(length(imputedsim111)%/%rubindatasets2)) {
      ...
      #rubin-validating real algorithm
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed11[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits1,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed12[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits2,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed13[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits3,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed21[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits1,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed22[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits2,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed23[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits3,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed31[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits1,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed32[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits2,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed33[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits3,datinpool=rubindatasets2,logtrans = TRUE)
      
      #rubin-validating real control
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont1[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree1,truevals,response,explanatory,frac=frac1,maxits=1,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont2[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree2,truevals,response,explanatory,frac=frac2,maxits=1,datinpool=rubindatasets2,logtrans = TRUE)
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont3[(1+(xb-1)*rubindatasets2):(xb*rubindatasets2)],datafree3,truevals,response,explanatory,frac=frac3,maxits=1,datinpool=rubindatasets2,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data1,truevals,response,explanatory,frac=frac1,maxits=0,datinpool=rubindatasets2,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data2,truevals,response,explanatory,frac=frac2,maxits=0,datinpool=rubindatasets2,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data3,truevals,response,explanatory,frac=frac3,maxits=0,datinpool=rubindatasets2,logtrans = TRUE)
    }
    ...
    for (xc in 1:(length(imputedsim111)%/%rubindatasets3)) {
      ...
      #rubin-validating real algorithm
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed11[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits1,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed12[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits2,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed13[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree1,truevals,response,explanatory,frac=frac1,maxits=maxits3,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed21[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits1,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed22[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits2,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed23[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree2,truevals,response,explanatory,frac=frac2,maxits=maxits3,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed31[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits1,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed32[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits2,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlist[[length(rubinlist)+1]]=rubinvalidator(imputed33[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree3,truevals,response,explanatory,frac=frac3,maxits=maxits3,datinpool=rubindatasets3,logtrans = TRUE)
      
      #rubin-validating real control
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont1[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree1,truevals,response,explanatory,frac=frac1,maxits=1,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont2[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree2,truevals,response,explanatory,frac=frac2,maxits=1,datinpool=rubindatasets3,logtrans = TRUE)
      rubinlistcont[[length(rubinlistcont)+1]]=rubinvalidator(imputedcont3[(1+(xc-1)*rubindatasets3):(xc*rubindatasets3)],datafree3,truevals,response,explanatory,frac=frac3,maxits=1,datinpool=rubindatasets3,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data1,truevals,response,explanatory,frac=frac1,maxits=0,datinpool=rubindatasets3,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data2,truevals,response,explanatory,frac=frac2,maxits=0,datinpool=rubindatasets3,logtrans = TRUE)
      ...
      rubinlistmice[[length(rubinlistmice)+1]]=rubinvalidator(micedats,data3,truevals,response,explanatory,frac=frac3,maxits=0,datinpool=rubindatasets3,logtrans = TRUE)
    }
